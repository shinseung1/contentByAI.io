...
async def get_generation_job(job_id: str):
    ...
    try:
        ...
        # db already imported as global instance
        db_job = db.get_generation_job(job_id)
        if not db_job:
            raise HTTPException(status_code=404, detail="Job not found")
        
        def safe_encode(value):
            """Safely encode any value to be JSON serializable."""
            if value is None:
                return None
        ...
        # Get workflow template name if exists
        workflow_template_name = None
        if hasattr(db_job, 'workflow_template_id') and db_job.workflow_template_id:
            try:
                workflow_template = db.get_workflow_template_by_id(db_job.workflow_template_id)
                if workflow_template:
                    workflow_template_name = safe_encode(workflow_template.name)
            except Exception as e:
                print(f"DEBUG: Error getting workflow template: {e}")
                pass
        
        # Return complete job info directly from database
        return {
            "job_id": safe_encode(db_job.job_id),
            "provider": safe_encode(db_job.provider),
            "status": safe_encode(db_job.status),
            "message": safe_encode(db_job.topic),
            "progress": (db_job.progress or 0) / 100.0,
            "created_at": safe_encode(str(db_job.created_at)) if db_job.created_at else None,
            "completed_at": safe_encode(str(db_job.updated_at)) if db_job.status in ['completed', 'failed'] and db_job.updated_at else None,
            "tone": safe_encode(db_job.tone),
            "word_count": db_job.word_count or 0,
            "content": safe_encode(db_job.content) if db_job.status == 'completed' else None,
            "error": safe_encode(db_job.error_message),
            "target_language": safe_encode(db_job.target_language),
            "include_images": bool(db_job.include_images),
            "workflow_template_id": getattr(db_job, 'workflow_template_id', None),
...
async def test_endpoint():
    ...
    return {"message": "Test successful", "jobs": []}

@router.get("/jobs-simple")
async def list_jobs_simple():
    """Simple jobs list without database."""
    return [
        {
            "job_id": "test-1",
            "status": "completed",
...
async def list_generation_jobs():
    ...
    try:
        ...
        def safe_encode(value):
            ...
            if isinstance(value, str):
                try:
                    # Remove or replace problematic characters for JSON serialization
                    import re
                    # First encode/decode to clean up encoding issues
                    cleaned = value.encode('utf-8', errors='ignore').decode('utf-8')
                    ...
                    # Additional safety - ensure JSON serializable
                    import json
                    json.dumps(cleaned)  # Test serialization
                    return cleaned
                except:
        ...
        for db_job in db_jobs:
            ...
            job_data = {
                ...
                "target_language": safe_encode(db_job.target_language),
                "include_images": bool(db_job.include_images),
                "content": "available" if db_job.status == 'completed' and db_job.content else None,
                "workflow_template_id": getattr(db_job, 'workflow_template_id', None),
                "workflow_template_name": workflow_template_name
            }
            jobs.append(job_data)
        
        # Sort by updated_at descending (newest first) - but jobs are already sorted from database
        # jobs.sort(key=lambda x: x.get('created_at', '') or '', reverse=True)
        return jobs
        
    except Exception as e:
        import traceback
        print(f"DEBUG: Error in list_generation_jobs: {e}")
