...
const GrokPage: React.FC = () => {
  ...
  const loadHistory = async () => {
    ...
    try {
      const jobs = await api.listGenerationJobs('grok')
      setHistoryJobs(jobs)
    } catch (error) {
  ...
  const handleGenerate = async (values: any) => {
    ...
    try {
      ...
      console.log('Grok generation request:', request)
      
      // Start generation
      const jobResponse = await api.generateContent(request)
      setCurrentJob(jobResponse.jobId)
      
      message.success(`Grok 콘텐츠 생성이 시작되었습니다! (Job ID: ${jobResponse.jobId})`)
  ...
  return (
    <div>
      <div className="page-header">
        <Title level={1}>
          <ExperimentOutlined /> Grok 콘텐츠 생성
        </Title>
        <Text type="secondary">X.AI의 Grok AI를 사용하여 창의적이고 유머러스한 콘텐츠를 생성합니다</Text>
      </div>
      ...
      <Card 
        title={
          <Space>
            <ExperimentOutlined style={{ color: '#1DA1F2' }} />
            새로운 콘텐츠 생성
          </Space>
        }
        style={{ marginTop: 24 }}
      >
        ...
        >
          ...
          >
            <TextArea
              rows={3}
              placeholder="Grok AI로 생성할 콘텐츠의 주제를 구체적으로 입력해주세요. 예: '트위터에서 화제가 된 재미있는 AI 트렌드와 유머러스한 관점'"
            />
          </Form.Item>
          ...
          <Row gutter={16}>
            <Col span={12}>
              <Form.Item name="tone" label="톤 앤 매너">
                <Select>
                  <Select.Option value="humorous">유머러스</Select.Option>
                  <Select.Option value="witty">재치있는</Select.Option>
                  ...
                  <Select.Option value="sarcastic">풍자적</Select.Option>
                </Select>
              </Form.Item>
            </Col>
            <Col span={12}>
      ...
      {(jobStatus?.result || jobStatus?.content) && (
        <Card 
          title={
            <Space>
              <EyeOutlined style={{ color: '#1DA1F2' }} />
              생성된 콘텐츠
            </Space>
          }
          style={{ marginTop: 24 }}
      ...
      <Card 
        title={
          <Space>
            <HistoryOutlined style={{ color: '#1DA1F2' }} />
            Grok 생성 히스토리
          </Space>
        }
        style={{ marginTop: 24 }}
