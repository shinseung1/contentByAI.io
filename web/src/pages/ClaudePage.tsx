...
const ClaudePage: React.FC = () => {
  ...
  const loadTestResults = async () => {
    try {
      const results = await ApiService.getTestResults('claude');
  ...
  const loadStats = async () => {
    try {
      const providerStats = await ApiService.getProviderStats('claude');
  ...
  const loadGenerationJobs = async () => {
    try {
      const jobs = await ApiService.getGenerationJobs('claude');
  ...
  const runTest = async () => {
    ...
    try {
      const result = await ApiService.testAiApi('claude', testPrompt);
      
      if (result.success) {
        message.success('Claude API 테스트가 성공했습니다!');
        loadTestResults();
        loadStats();
      } else {
        message.error('Claude API 테스트가 실패했습니다.');
      }
    } catch (error) {
  ...
  const presetPrompts = [
    ...
    { label: '분석적 글쓰기', value: '현대 사회에서 인공지능의 역할과 영향에 대해 분석적으로 설명해주세요.' },
    { label: '창의적 스토리텔링', value: '미래의 도시에서 벌어지는 흥미로운 이야기를 창작해주세요.' },
    { label: '논리적 설명', value: '블록체인 기술의 작동 원리를 단계별로 자세히 설명해주세요.' },
  ];
  ...
  return (
    <div style={{ padding: '24px', maxWidth: '1200px', margin: '0 auto' }}>
      <div style={{ marginBottom: 24 }}>
        <Title level={1}>
          <span style={{ color: '#8B5A2B' }}>🧠</span> Anthropic Claude
        </Title>
        <Paragraph type="secondary" style={{ fontSize: '16px' }}>
          Anthropic의 고성능 AI 모델 Claude를 사용하여 고품질 콘텐츠를 생성할 수 있습니다.
          뛰어난 추론 능력과 안전성을 바탕으로 복잡한 작업도 수행합니다.
        </Paragraph>
      </div>
          ...
          >
            <Space direction="vertical" style={{ width: '100%' }}>
              ...
              <Button
                ...
                style={{ width: '100%', backgroundColor: '#8B5A2B', borderColor: '#8B5A2B' }}
                size="large"
              >
                {isTestLoading ? '테스트 실행 중...' : 'Claude API 테스트'}
              </Button>
            </Space>
          </Card>
        </Col>
