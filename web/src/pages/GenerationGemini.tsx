...
const GenerationGemini: React.FC = () => {
  ...
  const loadHistory = async () => {
    ...
    try {
      const jobs = await api.listGenerationJobs('gemini')
      setHistoryJobs(jobs)
    } catch (error) {
  ...
  const handleGenerate = async (values: any) => {
    ...
    try {
      ...
      message.success(`Gemini 콘텐츠 생성이 시작되었습니다! (Job ID: ${jobResponse.jobId})`)
  ...
  return (
    <div>
      ...
      <Card 
        title={
          <Space>
            <HistoryOutlined style={{ color: '#4285f4' }} />
            Gemini 생성 히스토리
          </Space>
        }
        style={{ marginTop: 24 }}
